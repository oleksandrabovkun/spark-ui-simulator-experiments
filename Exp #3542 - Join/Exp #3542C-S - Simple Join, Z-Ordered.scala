...
// Dataset has about 813 partions, use 832 to maintain factor of cores
...
val resultsDF = trxDF
  .join(ctyDF, ctyDF("city_id") === trxDF("z_city_id")) // Join by city_id
...
val trxDF = spark
  ...
  .hint("skew", "z_city_id")                            // Required to avoid Spill and/or Executor-OOM 

val resultsDF = trxDF
  .join(ctyDF, $"city_id" === $"z_city_id")             // Join by city_id
...
val trxDF = spark
  .read.format("delta").load(trxPath)                   // Load the transactions table
  //.hint("skew", "z_city_id")                          // Not required with AQE
