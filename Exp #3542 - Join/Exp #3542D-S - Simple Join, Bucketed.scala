...
val ctyDF = spark.read.table(ctyTable)                    // Load the city table

val trxDF = spark.read.table(trxTable)                    // Load the transactions table

val resultsDF = trxDF
  .join(ctyDF, ctyDF("b_city_id") === trxDF("b_city_id")) // Join by city_id
...
val trxDF = spark
  .read.table(trxTable)                                   // Load the transactions table
  .hint("skew", "b_city_id")                              // Required to avoid Spill and/or Executor-OOM
...
val trxDF = spark.read.table(trxTable)                    // Load the transactions table
  //.hint("skew", "b_city_id")                            // Not required with AQE
